  ...
  CommandLineRunner init(GolfTournamentRepository repo, GolfTournamentService service) {
    return args -> {
      if (repo.count() == 0) {
        ...
        service.addGolferToTournament(tournamentId, new GolferRequest("Rory McIlroy"));
        service.addGolferToTournament(tournamentId, new GolferRequest("Phil Mickelson"));
        service.addGolferToTournament(tournamentId, new GolferRequest("Brooks Koepka"));
        service.addGolferToTournament(tournamentId, new GolferRequest("Tiger Woods"));
        service.addGolferToTournament(tournamentId, new GolferRequest("Dustin Johnson"));
